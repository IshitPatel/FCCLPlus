...
class FedLeaDigits(FederatedDataset):
    NAME = 'fl_digits'
    SETTING = 'domain_skew'
    DOMAINS_LIST = ['mnist', 'usps', 'svhn']
    percent_dict = {'mnist': 0.0023, 'usps': 0.013, 'svhn': 0.13}
            ...
            if domain == 'syn':
                ...
                if domain in ['mnist', 'usps']:
                    train_dataset = MyDigits(data_path(), train=True,
                                             download=False, transform=sin_chan_nor_transform, data_name=domain)
                else:
                    train_dataset = MyDigits(data_path(), train=True,
            ...
            if domain == 'syn':
                ...
                if domain in ['mnist', 'usps']:
                    test_dataset = MyDigits(data_path(), train=False,
                                            download=False, transform=sin_chan_test_transform, data_name=domain)
                else:
